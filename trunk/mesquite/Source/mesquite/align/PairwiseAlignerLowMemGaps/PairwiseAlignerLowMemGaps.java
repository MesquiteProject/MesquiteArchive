...
public class PairwiseAlignerLowMemGaps extends TwoSequenceAligner {
	...
	public long[][] alignSequences(long[] sequence1, long[] sequence2, boolean returnAlignment, MesquiteNumber score, CommandRecord commandRec) {
		...
		pa = new PairwiseAlignerBasicHelper(sequence1,sequence2,true);
		helper = pa.getLinearSpaceAlignmentHelper(true,!returnAlignment);
		...
		if (!returnAlignment) { // no need to  recover the alignment
			...
			PairwiseAlignerLowMemHelper lowmemHelper =new PairwiseAlignerLowMemHelper(pa,helper,true); 
